...
const Testpage = () => {
  ...
  const [oteruser, setoteruser] = useState("");
  const [socket, setso] = useState({});
  const [currentroom, setcurroom] = useState();
  const [pid, setpid] = useState();
  ///////////////////////////////////////bucket delete
  const joinRoom = async (pid, usernamea,oteruser) => {
    if (matchpeople) {
  ...
  return (
    ...
    >
      <nav className="bg-gradient-to-r from-gray-800 via-brown to-black text-white p-3">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-4  justify-between">
            <p className="text-2xl">𝙸𝚝 𝚛𝚒𝚍𝚎𝚜</p>
            <Button
              className="mt-1"
              color="inherit"
              sx={{
          ...
          <div className="flex items-center space-x-4">
            <button onClick={handleClickl} className="rounded-full border border-light-rose-red bg-white text-black px-3 py-1  hover:text-white hover:bg-red-600">
              Back
            </button>
          </div>
        </div>
                    ...
                    {matchpeople && matchpeople.length > 0 ? (
                      matchpeople.map((item, index) => (
                        <div key={index}>
                          ...
                          >
                            <Button onClick={() => joinRoom(item.id, username,item.name)}>
                              {item.name}
                              <sub>
                                {item.notif ? (
                                  <>
